...
export function Viewer(props: Props) {
  ...
  useEffect(() => {
    ...
    const fit = (selected?: string) => {
      ...
      const radius = Math.max(bounds.getSize(new T.Vector3()).length(), 0.2);
      camera.position
        .copy(center)
        .add(
          new T.Vector3(0.9, 0.8, 1.1).normalize().multiplyScalar(radius * 1.5),
        );
    ...
    const pointerUp = (e: PointerEvent) => {
      ...
      if (hits.length) latest.current.onSelect(hits[0].object.userData.record);
    };
    ...
    Promise.all(
      ["a.glb", "b.glb"].map(async (file, side) => {
        ...
        root.traverse((node) => {
          ...
          while (owner && !owner.userData.sce_entity) owner = owner.parent;
  ...
  useEffect(() => {
    ...
    for (const mesh of s.meshes) {
      ...
      mesh.visible =
        opacity[side] > 0.01 && (!props.changedOnly || status !== "unchanged");
    ...
    if (selected?.before && selected?.after) {
      ...
      const b = props.report.after.entities.find((e) => e.id === selected.after)
        ?.extensions.blender?.evaluated_world;
      if (a && b) {
